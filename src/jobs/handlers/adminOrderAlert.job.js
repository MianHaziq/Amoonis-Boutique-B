 ...
 *
 * Data: { orderId, totalAmount?, currency?, buyerId? }
 *
 * Fans out one push.send job per active staff member. These are OPERATIONAL alerts
 * (prefKey: null) so they bypass the staff member's personal customer notification
 * preferences — an admin can't accidentally silence new-order alerts by turning off
 * their own "order updates" toggle. The push carries `type: ORDER_PLACED` so the app
 * routes admins/managers to the admin order screen (it branches on the logged-in role).
 *
...
async function handle(data) {
  const { orderId, totalAmount, currency = 'AED', buyerId } = data;
  if (!orderId) {
  ...
  const body = `Order #${shortRef(orderId)} placed${amount}.`;
